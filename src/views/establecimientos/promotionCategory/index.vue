...
const model = reactive(new PromotionCategory(null, null, null, null));
const modelN = reactive(new PromotionCategory(null, null, null, null));
...
async function consultarRegistros() {
  registros.value = [];
  try {
    const response = await api.get(`cms/promotion-category/${page.value}/${size}`, {
    ...
    if (Array.isArray(response.data.content) && response.data.content.length > 0) {
      ...
      console.log('response.data.numberOfElements',numberOfElements.value);
...
async function searchRegistros() {
  ...
  try {
    const response = await api.post("cms/promotion-category/search",datosEnviar, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
async function paginateRegistros(pageNew:number) {
  ...
  try {
    const response = await api.get(`cms/promotion-category/${pageNew}/${size}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    console.log('content:',response.data.content);
    if (Array.isArray(response.data.content) && response.data.content.length > 0) {
...
function modalAgregar(){
  ...
  title_modal.value = "Agregar Categoria";
  editar.value = false;
}
...
function modalEditar(mo:any){
  editar.value = true;
  showModal.value = true;
  title_modal.value = "Modificar Categoria";
  modelN.name = mo.name;
  modelN.id = mo.id;
...
async function guardar(){
  ...
  if (result?.valid) {
    ...
    try {
      const response = await api.post("cms/promotion-category/create",modelN, {
        headers: { 
      ...
      if (response.data.data) {
        ...
        if(response.data.status===201 && response.data.data){
          ...
          Object.assign(modelN, new PromotionCategory(null, null, null, null, null));
...
async function modificar(){
  ...
  if (result?.valid ) {
    console.log("Formulario válido ✅", modelN);
    try {
      const response = await api.put(`cms/promotion-category/update/${modelN.id}`,modelN, {
        headers: { 
          Authorization: `Bearer ${token}`,
        },
      });
...
async function fnDelete(mo: any) {
  ...
  try {
    const response = await api.delete(`cms/promotion-category/delete/${mo.id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
...
<template>
  <v-row >
    <v-col cols="12">
      <UiChildCard title="Buscar Categorías" class="py-1">
  ...
  <v-row class="justify-space-between align-center px-3 py-3 bg-white rounded-b-lg mx-0">
    <v-col cols="12" sm="12" md="6" lg="6" class="d-flex align-items-stretch" >
      <div class="text-h5">Categorias ({{ totalElements }})</div>
    </v-col>
    <v-col cols="12" sm="12" md="6" lg="6" class="d-flex align-items-stretch" >
