...
const model = reactive(new SectionType(null, null, null, null, null, null));
const modelN = reactive(new SectionType(null, null, null, null, null, null));
...
route.value = "section-type";
pageTitle.value = "Tipo de sección";
...
function modalEditar(mo:any){
  ...
  modelN.logo = mo.logo;
  modelN.id = mo.id;  
  image.value = url_image.value + mo.logo;
}
...
async function onImageChange(event: Event){
  ...
  if (target.files && target.files[0]) {
    ...
    formData.append("logo", file.value as File);       
    try {
      const response = await api.post(`cms/${route.value}/image`,formData, {
        headers: { 
      ...
      if (response.data.data) {
        if(response.data.status===201 && response.data.data){
          ...
          modelN.logo = response.data.data;        
        }
      } else {
    ...
    try {
      ...
      if (response.data.data) {
        ...
        if(response.data.status===201 && response.data.data){
          ...
          Object.assign(modelN, new SectionType(null, null, null, null, null, null));
          image.value = null;
          file.value = null;
          date.value = null;
        }
...
<template>
  ...
  <v-row>
    ...
    <v-col v-for="(item, index) in registros" :key="item.id || index" cols="12" sm="12" lg="4" class="d-flex align-items-stretch" >
        ...
        <CardsMedia :img="url_image+item.logo" fit="contain"
          :title="item.title" 
        ...
        <v-form ref="formAddFacility" v-model="valid" lazy-validation>
          <v-row class="mt-1 mb-1" > 
            ...
            <v-col cols="12" sm="6" md="6" lg="4" class="d-flex align-center">
              <v-file-input accept="image/*" label="Subir imagen" prepend-icon="mdi-image" @change="onImageChange" class="file-input-custom" 
