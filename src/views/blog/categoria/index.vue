...
const model = reactive(new Category(null, null, null, null));
const modelN = reactive(new Category(null, null, null, null));
...
async function consultarRegistros() {
  registros.value = [];
  try {
    const response = await api.get(`cms/category/${page.value}/${size}`, {
...
async function searchRegistros() {
  ...
  try {
    const response = await api.post("cms/category/search",datosEnviar, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
async function paginateRegistros(pageNew:number) {
  ...
  try {
    const response = await api.get(`cms/category/${pageNew}/${size}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    console.log('content:',response.data.content);
    if (Array.isArray(response.data.content) && response.data.content.length > 0) {
...
async function guardar(){
  ...
  if (result?.valid) {
    ...
    try {
      const response = await api.post("cms/category/create",modelN, {
        headers: { 
      ...
      if (response.data.data) {
        ...
        if(response.data.status===201 && response.data.data){
          ...
          Object.assign(modelN, new Category(null, null, null, null, null));
...
async function modificar(){
  ...
  if (result?.valid ) {
    console.log("Formulario válido ✅", modelN);
    try {
      const response = await api.put(`cms/category/update/${modelN.id}`,modelN, {
        headers: { 
          Authorization: `Bearer ${token}`,
        },
      });
...
async function fnDelete(mo: any) {
  ...
  try {
    const response = await api.delete(`cms/category/delete/${mo.id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
