...
const slug_menu = ref('accidentes-personales');
const menu_id = ref(0);
...
async function modalEditarPageContent(mo:any){
  ...
  await nextTick();
  await initEditorDescPageContent();
  await initEditorTitlePageContent();
}


let editorInstanceDesc:any = null;
let editorInstanceDescPageContent:any = null;
let editorInstanceTitlePageContent:any = null;
...
async function initEditorTitlePageContent() {
  tinymce.init({
    ...
    setup: (editorTitle) => {
      editorInstanceTitlePageContent = editorTitle;
