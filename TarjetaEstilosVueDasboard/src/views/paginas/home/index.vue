...
const registrosShortcuts = ref<any[]>([]);
const registrosCta = ref<any[]>([]);
const registrosFaq = ref<any[]>([]);
const registrosFacility = ref<any[]>([]);
...
const modelShortcuts = reactive(new Shortcut(null, null, null, null, null, null, null));
const modelNShortcuts = reactive(new Shortcut(null, null, null, null, null, null, null));
...
const modelFacility = reactive(new Facility(null, null, null, null, null, null, null));
const modelNFacility = reactive(new Facility(null, null, null, null, null, null, null));
...
const totalElementsShortcuts = ref(0);
const numberOfElementsShortcuts = ref(0);
const totalPagesShortcuts = ref(0);
...
const formAddShortcuts = ref<VForm | null>(null);
...
const editarShortcuts = ref(false);
const editarCta = ref(false);
const editarFaq = ref(false);
const editarFacility = ref(false);
...
const imageFacility = ref<string | null>(null);
const file = ref<File|null>(null);
const fileFacility = ref<File|null>(null);
...
routeShortcuts.value = "shortcuts";
routeCta.value = "cta";
routeFaq.value = "faq";
routeFacility.value = "facility";
...
async function searchGetMenuPromotion() {
  ...
  try {
    ...
    if (Array.isArray(response.data.content) && response.data.content.length > 0) {
      ...
      promotion_menu_id.value = response.data.content[0].id; 

      modelPromotion.menu_id = response.data.content[0].id;
      modelNPromotion.menu_id = response.data.content[0].id;
      modelPromotion.home = true;
      modelNPromotion.home = true;
      modelNPromotion.featured = true;
      consultarDepartamentos();
      getCategories();
      searchRegistrosPromotions();
    } else {
      showmessage.value = true;
    }
  } catch (error: any) {
...
async function searchRegistrosPromotions() {
  registrosPromotion.value = [];  
  modelPromotion.menu_id = promotion_menu_id.value;
  modelNPromotion.featured = true;
  modelNPromotion.home = true;
  try {
...
async function searchRegistrosShortcuts() {
  registrosShortcuts.value = [];  
  modelShortcuts.menu_id = menu_id.value;
  try {
    const response = await api.post(`cms/${routeShortcuts.value}/search`,modelShortcuts, {
      headers: { Authorization: `Bearer ${token}` },
    });
    console.log('content:',response.data.content);
    if (Array.isArray(response.data.content) && response.data.content.length > 0) {
      console.log('content:',response.data.content);
      registrosShortcuts.value = response.data.content;
      totalPagesShortcuts.value = response.data.totalPages;
      totalElementsShortcuts.value = response.data.totalElements;
      numberOfElementsShortcuts.value = response.data.numberOfElements;
    } else {
      showmessage.value = true;
    }
  } catch (error: any) {
...
async function searchRegistrosFacility() {
  registrosFacility.value = [];
  modelFacility.menu_id = menu_id.value;
  try {
    const response = await api.post(`cms/${routeFacility.value}/search`,modelFacility, {
      headers: { Authorization: `Bearer ${token}` },
    });
    console.log('content:',response.data.content);
    if (Array.isArray(response.data.content) && response.data.content.length > 0) {
      console.log('content:',response.data.content);
      registrosFacility.value = response.data.content;
      totalPagesFacility.value = response.data.totalPages;
      totalElementsFacility.value = response.data.totalElements;
      numberOfElementsFacility.value = response.data.numberOfElements
    } else {
      showmessage.value = true;
    }
  } catch (error: any) {
...
async function modalAgregarBanner(){ 
  ...
  title_modal.value = "Agregar Banner Home";
...
async function modalAgregarPromotion(){ 
  ...
  title_modalPromotion.value = "Agregar Promoción Home";
...
async function modalAgregarShortcuts(){ 
  showModalShortcuts.value = true;
  title_modalShortcuts.value = "Agregar Atajo Home";
  showEditorShortcuts.value = true;
  editarShortcuts.value = false;
  await nextTick(); 
  console.log('modelShortcuts:',modelNShortcuts);
}

async function modalAgregarCta(){ 
  showModalCta.value = true;
  title_modalCta.value = "Agregar Cta Home";
...
async function modalAgregarFaq(){ 
  ...
  title_modalFaq.value = "Agregar preguntas home";
...
async function modalEditarBanner(mo:any){
  ...
  editar.value = true;
  showModalBanner.value = true;
  title_modal.value = "Modificar Banner Home";
  showEditor.value = true;
...
async function modalEditarShortcuts(mo:any){
  
  editarShortcuts.value = true;
  showModalShortcuts.value = true;
  title_modalShortcuts.value = "Modificar atajo";
  showEditorShortcuts.value = true;
  modelNShortcuts.title = mo.title;
  modelNShortcuts.icon = mo.icon;  
  modelNShortcuts.id = mo.id;
  modelNShortcuts.menu_id = mo.menu.id;
  await nextTick();
...
function closeModalShortcuts(){
  showModalShortcuts.value = false; showEditorShortcuts.value = false;
  Object.assign(modelNShortcuts, new Shortcut(null, null, null, null, null, null, null));
  image.value = null;
  file.value = null;
  date.value = null;
    ...
    try {
      ...
      if (response.data.data) {
        ...
        if(response.data.status===201 && response.data.data){          
          ...
          flassmessagesuccess(response.data.message,20);
          showModalBanner.value = false; showEditor.value = false;
          Object.assign(modelNBanner, new Promotion(null, null, null, null, null, null, null, null, null, null, null,null,null));
          image.value = null;
          file.value = null;
          date.value = null;
          logo.value = null;
...
async function guardarPromotion(){
  ...
  modelNPromotion.menu_id = promotion_menu_id.value;
  modelNPromotion.home = true;
  modelNPromotion.featured = true;
  if (result?.valid && ckValid && ckValidTerm ) {
    try {
...
async function guardarShortcuts(){
  console.log('guardar');      
  const result = await formAddShortcuts.value?.validate();
  console.log('modelNShortcuts:', modelNShortcuts);
  modelNShortcuts.menu_id = menu_id.value;
  if (result?.valid ) {
    try {
      const response = await api.post(`cms/${routeShortcuts.value}/create`,modelNShortcuts, {
        headers: { 
          Authorization: `Bearer ${token}`
        },
      });
      ...
      if (response.data.data) {
        console.log('content is array:',response.data.data);         
        if(response.data.status===201 && response.data.data){          
          registrosShortcuts.value.unshift(response.data.data);
          createdId.value = response.data.data.id;
          setTimeout(() => (createdId.value = 0), 2000) 
          //console.log('ingreso');
          flassmessagesuccess(response.data.message,20);
...
async function modificarShortcuts(){
  console.log('modiifcar');  
  console.log('date:',date); 
  const result = await formAddShortcuts.value?.validate()
  modelNShortcuts.menu_id = menu_id.value;
  if (result?.valid) {
    console.log("Formulario válido atajo ✅", modelNShortcuts);
    try {
      const response = await api.put(`cms/${routeShortcuts.value}/update/${modelNShortcuts.id}`,modelNShortcuts, {
        headers: { 
          Authorization: `Bearer ${token}`
        },
      });
      ...
      if (response.data.data) {
        console.log('content is array:',response.data.data); 
        const index = registrosShortcuts.value.findIndex(r => r.id === response.data.data.id);
        
        if(response.data.status===200 && response.data.data){
          updatedId.value = response.data.data.id;
          registrosShortcuts.value[index] = response.data.data;
          setTimeout(() => (updatedId.value = 0), 2000) 
          //console.log('ingreso');
          flassmessagesuccess(response.data.message,20);
          showModalShortcuts.value = false; showEditor.value = false;
          Object.assign(modelNShortcuts, new Shortcut(null, null, null, null, null, null, null));
          image.value = null;
          file.value = null;
          date.value = null;
...
async function fnDeleteShortcuts(mo: any) {
  console.log('Eliminar:', mo);
  const willDelete = await swal({
    title: "¿Está seguro de eliminar el registro?",
    text: "El registro no podrá ser recuperado!",
  ...
  try {
    const response = await api.delete(`cms/${routeShortcuts.value}/delete/${mo.id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "multipart/form-data"
      },
    ...
    if (response.data.status === 200) {
      ...
      setTimeout(() => { 
        updatedId.value = 0; 
        setTimeout(() => { 
          registrosShortcuts.value = registrosShortcuts.value.filter((r) => r.id !== mo.id);
        }, 1000); 
      }, 5000);
      flassmessagesuccess(response.data.message, 5000);      
    } else {
...
async function modalAgregarFacility(){
  showModalFacility.value = true;
  title_modalFacility.value = "Agregar Facilidad";
  editar.value = false;
}

async function modalEditarFacility(mo:any){
  editar.value = true;
  showModalFacility.value = true;
  title_modalFacility.value = "Modificar Facilidad";
  showEditorFacility.value = true;
  modelNFacility.title = mo.title;
  modelNFacility.label = mo.label;
  modelNFacility.button_label = mo.buttonLabel;
  imageFacility.value = url_image.value + mo.image;
  modelNFacility.id = mo.id;  
}

async function onFileChangeFacility(event: Event){
  const target = event.target as HTMLInputElement
  if (target.files && target.files[0]) {
    fileFacility.value = target.files[0] 
    const reader = new FileReader()
    reader.onload = e => {
      imageFacility.value = e.target?.result as string
    }
    reader.readAsDataURL(fileFacility.value) 
    const formData = new FormData();
    formData.append("image", fileFacility.value as File); 
      
    try {
      const response = await api.post(`cms/${routeFacility.value}/image`,formData, {
        headers: { 
          Authorization: `Bearer ${token}`
        },
      });
      ...
      if (response.data.data) {
        if(response.data.status===201 && response.data.data){
          flassmessagesuccess(response.data.message,20);
          modelNFacility.image = response.data.data;        
        }
      } else {
        showmessage.value = true;
      }
...
async function guardarFacility(){
  console.log('guardar');
  console.log('date:',date);  
  modelNFacility.menu_id = menu_id.value;

  const result = await formAddFacility.value?.validate();
  if (result?.valid) {
    try {
      const response = await api.post(`cms/${routeFacility.value}/create`,modelNFacility, {
        headers: { 
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      if (response.data.data) {
        ...
        if(response.data.status===201 && response.data.data){
          ...
          registrosFacility.value.unshift(response.data.data);
          createdId.value = response.data.data.id;
          setTimeout(() => (createdId.value = 0), 2000) 
          //console.log('ingreso');
          flassmessagesuccess(response.data.message,20);
          showModalFacility.value = false; showEditor.value = false;
          Object.assign(modelNFacility, new Facility(null, null, null, null, null, null, null, null));
          imageFacility.value = null;
          fileFacility.value = null;
          date.value = null;
        }
      } else {
        showmessage.value = true;
...
async function modificarFacility(){
  console.log('modiifcar');  
  console.log('date:',date);
  modelNFacility.menu_id = menu_id.value;
  ...
  if (result?.valid) {
    ...
    try {
      const response = await api.put(`cms/${routeFacility.value}/update/${modelNFacility.id}`,modelNFacility, {
        headers: { 
          Authorization: `Bearer ${token}`
        },
      });
      ...
      if (response.data.data) {
        console.log('content is array:',response.data.data); 
        const index = registrosFacility.value.findIndex(r => r.id === response.data.data.id);
        
        if(response.data.status===200 && response.data.data){
          updatedId.value = response.data.data.id;
          registrosFacility.value[index] = response.data.data;
          setTimeout(() => (updatedId.value = 0), 2000) 
          //console.log('ingreso');
          flassmessagesuccess(response.data.message,20);
          showModalFacility.value = false; showEditor.value = false;
...
async function fnDeleteFacility(mo: any) {
  ...
  try {
    const response = await api.delete(`cms/${routeFacility.value}/delete/${mo.id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "multipart/form-data"
      },
    ...
    if (response.data.status === 200) {
      ...
      setTimeout(() => { 
        updatedId.value = 0; 
        setTimeout(() => { 
          registrosFacility.value = registrosFacility.value.filter((r) => r.id !== mo.id);
        }, 1000); 
      }, 5000);
      flassmessagesuccess(response.data.message, 5000);      
    } else {
...
<template>
  <v-container>
    ...
    <v-tabs v-model="tab" bg-color="primary" dark>
      ...
      <v-tab value="promociones" @click="searchGetMenuPromotion">Promociones</v-tab>
      <v-tab value="atajos" @click="searchRegistrosShortcuts">Atajos</v-tab>
      <v-tab value="cta" @click="searchRegistrosCta">Cta</v-tab>
      <v-tab value="faq" @click="searchRegistrosFaq">Preguntas Frequentes</v-tab> 
      <v-tab value="facilidades" @click="searchRegistrosFacility">Facilidades</v-tab> 
    </v-tabs>
    ...
    <v-window v-model="tab" class="mt-4">
      ...
      <v-window-item value="promociones">
        <v-card flat>
          <v-card-text>
      ...
      <v-window-item value="atajos">
        <v-card flat>
          <v-card-text>
            <v-form @submit.prevent="searchRegistrosShortcuts()">
              <v-row dense>
                <v-col cols="12" sm="4">
                  <v-text-field
                    v-model="modelPromotion.title"
                ...
                <v-col cols="12" sm="4" class="d-flex justify-end">
                  ...
                  <v-btn text="Agregar" color="info" @click="modalAgregarShortcuts">
                    <template v-slot:prepend>
                      <div class="d-flex align-center justify-center" >
                        <Icon icon="solar:camera-add-line-duotone" width="24" height="24" />
                      </div>
            ...
            <v-row dense class="mt-4">
              <v-col
                v-for="(item, index) in registrosShortcuts"
                :key="index"
                cols="12" sm="6" md="4"
              >
                <v-card class="h-100"
                ...
                >
                  ...
                  <v-card-subtitle v-html="item.subtitle"></v-card-subtitle>
                  <v-card-actions>
                    <v-btn text="Editar" color="lightinfo" elevation="5" variant="elevated" class="no-hover-shadow " @click="modalEditarShortcuts(item)"> 
                      <template v-slot:prepend>
                        <div class="d-flex align-center justify-center" >
                          <Icon icon="solar:gallery-edit-line-duotone" width="24" height="24" />
                        </div>
                    ...
                    <v-btn text="Eliminar" color="error" elevation="5" variant="elevated" class="no-hover-shadow " @click="fnDeleteShortcuts(item)">
                      <template v-slot:prepend>
                        <div class="d-flex align-center justify-center" >
                          <Icon icon="solar:trash-bin-trash-line-duotone" width="24" height="24" />            
                        </div>        
              ...
              <v-col cols="12" class="bg-amber" v-if="registrosShortcuts.length==0">
                <div class="p-3 bg-white">
                  <Closable />
                </div>
              </v-col>
      ...
      <v-window-item value="facilidades">
        <v-card flat>
          <v-card-text>
            <v-form @submit.prevent="searchRegistrosFacility()">
              <v-row dense>
                <v-col cols="12" sm="4">
                  <v-text-field
                    v-model="modelFacility.title"
                ...
                <v-col cols="12" sm="4" class="d-flex justify-end">
                  ...
                  <v-btn text="Agregar" color="info" @click="modalAgregarFacility()">
                    <template v-slot:prepend>
                      <div class="d-flex align-center justify-center" >
                        <Icon icon="solar:camera-add-line-duotone" width="24" height="24" />
                      </div>
            ...
            <v-row dense class="mt-4">
              <v-col
                v-for="(item, index) in registrosFacility"
                :key="index"
                cols="12" sm="6" md="4"
              >
                <v-card class="h-100"
                ...
                >    
                  ...
                  <v-card-title v-html="item.title"></v-card-title>
                  <v-card-actions>
                    <v-btn text="Editar" color="lightinfo" elevation="5" variant="elevated" class="no-hover-shadow " @click="modalEditarFacility(item)"> 
                      <template v-slot:prepend>
                        <div class="d-flex align-center justify-center" >
                          <Icon icon="solar:gallery-edit-line-duotone" width="24" height="24" />
                        </div>
                    ...
                    <v-btn text="Eliminar" color="error" elevation="5" variant="elevated" class="no-hover-shadow " @click="fnDeleteFacility(item)">
                      <template v-slot:prepend>
                        <div class="d-flex align-center justify-center" >
                          <Icon icon="solar:trash-bin-trash-line-duotone" width="24" height="24" />            
                        </div>        
              ...
              <v-col cols="12" class="bg-amber" v-if="registrosFacility.length==0">
                <div class="p-3 bg-white">
                  <Closable />
                </div>
              </v-col>
  ...
  <v-dialog v-model="showModalShortcuts" scrim="primary" :persistent="true" max-width="1000px" transition="dialog-bottom-transition">
    <v-card>
      <v-card-title class="v-card-title sticky top-0 bg-white z-10 py-4 d-flex align-center">
        {{ title_modalShortcuts }}
        <v-spacer></v-spacer>
        <v-btn icon @click="closeModalShortcuts()" class="close-btn" elevation="3" color="error">
          <v-icon>mdi-close</v-icon>
        </v-btn>
      </v-card-title>      
      <v-card-text class="flex-grow-1 overflow-y-auto">
        <v-form ref="formAddShortcuts" v-model="valid" lazy-validation>
          <div class="v-row mt-1 mb-1">
            <div class="v-col-md-6 v-col-12">
              <v-text-field label="Titulo" placeholder="Titulo" v-model="modelNShortcuts.title" 
                :rules="[rules.required, rules.min3]"
                required 
              ></v-text-field>
            </div>            
            <div class="v-col-md-6 v-col-12">
              <v-text-field
                v-model="modelNShortcuts.icon"
                label="Icono" placeholder="Icono"
                color="primary" outlined
              ></v-text-field>
            </div>            
          </div>
      ...
      <v-card-actions class="sticky bottom-0 bg-white z-10">
        <v-btn text="Cancelar" color="error" elevation="5" variant="elevated" @click="closeModalShortcuts()">
          <template v-slot:prepend>
            <div class="d-flex align-center justify-center" >
              <Icon icon="line-md:close-circle-filled" width="24" height="24" />
            </div>        
        ...
        <v-btn text="Guardar" color="success" elevation="5" variant="elevated" v-if="!editarShortcuts" @click="guardarShortcuts">
          <template v-slot:prepend>
            <div class="d-flex align-center justify-center" >
              <Icon icon="line-md:clipboard-plus-twotone" width="24" height="24" />
            </div>        
        ...
        <v-btn text="Modificar" color="success" elevation="5" variant="elevated" v-if="editarShortcuts" @click="modificarShortcuts">
          <template v-slot:prepend>
            <div class="d-flex align-center justify-center" >
              <Icon icon="line-md:clipboard-plus-twotone" width="24" height="24" />
            </div>        
  ...
  <v-dialog v-model="showModalFacility" scrim="primary" :persistent="true" max-width="1000px" transition="dialog-bottom-transition">
    <v-card>
      <v-card-title class="v-card-title sticky top-0 bg-white z-10 py-4 d-flex align-center">
        {{ title_modal }}
        <v-spacer></v-spacer>
        <v-btn icon @click="closeModalFacility()" class="close-btn" elevation="3" color="error">
          <v-icon>mdi-close</v-icon>
        </v-btn>
      </v-card-title>      
      <v-card-text class="flex-grow-1 overflow-y-auto">
        <v-form ref="formAddFacility" v-model="valid" lazy-validation>
          <v-row class="mt-1 mb-1" > 
            <v-col cols="12" sm="6" md="6" lg="4" >
              <v-text-field label="Titulo" placeholder="Titulo" v-model="modelNFacility.title" 
                :rules="[rules.required, rules.min3]"
                required 
              ></v-text-field>
            </v-col>
            <v-col cols="12" sm="6" md="6" lg="4" >
              <v-text-field label="Etiqueta" placeholder="Etiqueta" v-model="modelNFacility.label" 
                :rules="[rules.required, rules.min3]"
                required 
              ></v-text-field>
            </v-col>
            <v-col cols="12" sm="6" md="6" lg="4" >
              <v-text-field label="Texto de botón" placeholder="texto de botón" v-model="modelNFacility.button_label" 
                :rules="[rules.required, rules.min3]"
                required 
              ></v-text-field>
            </v-col>
            <v-col cols="12" sm="6" md="6" lg="4" class="d-flex align-center">
              <v-file-input accept="image/*" label="Subir imagen Card" prepend-icon="mdi-image" @change="onFileChangeFacility" class="file-input-custom" 
                :rules="!editar ? [v => !!v || 'La imagen es obligatoria'] : []"
              />
            </v-col>
            <v-col cols="12" sm="6" md="6" lg="4" class="d-flex align-center">
              <v-img v-if="imageFacility" :src="imageFacility" max-width="300" class="rounded-xl" />
            </v-col>            
          </v-row>
        </v-form>
      </v-card-text>
      <v-card-actions class="sticky bottom-0 bg-white z-10">
        <v-btn text="Cancelar" color="error" elevation="5" variant="elevated" @click="closeModalFacility()">
          <template v-slot:prepend>
            <div class="d-flex align-center justify-center" >
              <Icon icon="line-md:close-circle-filled" width="24" height="24" />
            </div>        
        ...
        <v-btn text="Guardar" color="success" elevation="5" variant="elevated" v-if="!editar" @click="guardarFacility">
          <template v-slot:prepend>
            <div class="d-flex align-center justify-center" >
              <Icon icon="line-md:clipboard-plus-twotone" width="24" height="24" />
            </div>        
        ...
        <v-btn text="Modificar" color="success" elevation="5" variant="elevated" v-if="editar" @click="modificarFacility">
          <template v-slot:prepend>
            <div class="d-flex align-center justify-center" >
              <Icon icon="line-md:clipboard-plus-twotone" width="24" height="24" />
            </div>        
