...
const registrosSections = ref<any[]>([])
const url_image = ref(Global.url_image)
const model = reactive(new Menu(null, null, null, null, null, null, null, null, null, null));
const modelN = reactive(new Menu(null, null, null, null, null, null, null, null, null, null));
...
const routeSection = ref<string|null>(null);
const pageTitle = ref<string|null>(null);
const sections = ref<any[]>([])

route.value = "menu";
routeSection.value = "section-all";
...
function modalEditar(mo:any){
  ...
  modelN.icon = mo.icon;
  modelN.page = mo.page;
  modelN.product = mo.product;
  modelN.insurance = mo.insurance;
  modelN.sections = mo.sectionsMenus.map((item:any) => item.section.id);
  modelN.types = mo.menuType.map((item:any) => item.sectionType.id);
}
...
watch(() => modelN.title, (newTitle) => {
  modelN.slug = generateSlug(newTitle ?? '');
});
...
async function consultarSecciones() {
  registrosSections.value = [];

  try {
    const response = await api.get(`cms/${routeSection.value}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    console.log('content:',response.data);
    if (Array.isArray(response.data) && response.data.length > 0) {
      console.log('content:',response.data);
      registrosSections.value = response.data.map((tp:any) => ({
        title: tp.title,
        value: tp.id
      }))
    } else {
    ...
    try {
      ...
      if (response.data.data) {
        ...
        if(response.data.status===201 && response.data.data){
          ...
          Object.assign(modelN, new Menu(null, null, null, null, null, null, null, null, null, null));
          image.value = null;
          file.value = null;
          date.value = null;
...
<template>
      ...
      <UiChildCard :title="`Buscar ${pageTitle}`" class="py-1">
        <v-form @submit.prevent="searchRegistros()">
          <v-row class="mt-1 mb-1" > 
            ...
            <v-col cols="12" sm="6" md="6" lg="4" >
              <v-select chips multiple :items="registrosSections" label="Secciones" v-model="model.sections" />
            </v-col>
            <v-col cols="12" sm="6" md="6" lg="4" >
  ...
  <v-row>
    ...
    <v-col v-for="(item, index) in registros" :key="item.id || index" cols="12" sm="12" lg="4" class="d-flex align-items-stretch" >
        ...
        <CardsMedia  fit="contain"
          :title="item.title" :slug="item.slug" :sections="item.sectionsMenus" :tipoMenus="item.menuType" :pagina="item.page" :productos="item.products" 
      ...
      <v-card-text class="flex-grow-1 overflow-y-auto">
        <v-form ref="formAddFacility" v-model="valid" lazy-validation>
          <v-row class="mt-1 mb-1" > 
            ...
            <v-col cols="12" sm="12" md="12" lg="6" >
              <v-text-field label="Slug" placeholder="Slug" v-model="modelN.slug" 
                :rules="[rules.required, rules.min3]"
                required 
              ></v-text-field>
            </v-col>
            <v-col cols="12" sm="12" md="12" lg="6" >
              <v-text-field label="Icono" placeholder="Icono" v-model="modelN.icon" 
                :rules="[rules.required, rules.min3]"
                required 
              ></v-text-field>
            </v-col>
            ...
            <v-col cols="12" sm="12" md="12" lg="4" >
              <v-checkbox
                v-model="modelN.insurance"
                label="Seguros"
                color="primary"
              ></v-checkbox>
            </v-col>
            <v-col cols="12" sm="12" md="12" lg="4" >
              <v-select chips multiple :items="registrosSections" label="Secciones" v-model="modelN.sections" />
            </v-col>
            <v-col cols="12" sm="12" md="12" lg="4" >
              <v-select chips multiple :items="registrosType" label="Tipo de menu" v-model="modelN.types" />
            </v-col>
          </v-row>
        </v-form>
      </v-card-text>
